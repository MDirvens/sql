...
const insertMovieGenres = (
  ...
  var filtred = genreRows.filter(genreRow => genres.includes(genreRow.genre,))
  return `insert into ${MOVIE_GENRES} (movie_id, genre_id) values` + 
  filtred.map(genres => `(${movieId}, ${genres.id} )`).join(",")
};
...
const insertMovieActors = (
  ...
  var filtred = actorRows.filter(actorRow => actors.includes(actorRow.full_name,))
  return `insert into ${MOVIE_ACTORS} (movie_id, actor_id) values` + 
  filtred.map(actors => `(${movieId}, ${actors.id} )`).join(",")
};
...
const insertMovieDirectors = (
  ...
  var filtred = directorRows.filter(directorRow => directors.includes(directorRow.full_name,))
  return `insert into ${MOVIE_DIRECTORS} (movie_id, director_id) values` + 
  filtred.map(directors => `(${movieId}, ${directors.id} )`).join(",")
};
...
const insertMovieKeywords = (
  ...
  var filtred = keywordRows.filter(keywordRow => keywords.includes(keywordRow.keyword,))
  return `insert into ${MOVIE_KEYWORDS} (movie_id, keyword_id) values` + 
  filtred.map(keywords => `(${movieId}, ${keywords.id} )`).join(",")
};
...
const insertMovieProductionCompanies = (
  ...
  var filtred = productionCompanyRows.filter(productionCompanyRow => productionCompanies.includes(productionCompanyRow.company_name,))
  return `insert into ${MOVIE_PRODUCTION_COMPANIES} (movie_id, company_id) values` + 
  filtred.map(productionCompanies => `(${movieId}, ${productionCompanies.id} )`).join(",")
};
